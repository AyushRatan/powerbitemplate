...
# %%
instruction = '''* you're a Senior PowerBI developer. Your job is to identify the row numbers in the metadata sheet that match the requirement query. Carefully follow the given instructions given below to 

** make sure to apply those restriction and follow them directly and dont skip any of them:

output with no introduction, no explaintation, only row number

DONT MAKE ANY MISTAKES, check if you did any

only return row number, and nothing else.
...
#     messages=[
#         {"role": "system", "content": f"Given a metadata sheet of Power BI reports {table} and a new requirement query {query}, identify the row numbers in the metadata sheet that match the new requirement query"},
...
def openai_response(prompt,table):
    ...
    model="gpt-35-turbo", # model = "deployment_name".
    messages=[
        {"role": "system", "content": "You are a helpful assistant proficient in PowerBI. identify the row numbers in the metadata sheet that match the new requirement query. Give me a list with only row numbers. Output with no introduction, no explaintation, ONLY A PYTHON LIST OF TOP 5 ROW NUMBERS"},
...
query = 'Give me a report with Barchart.'#st.text_input('Enter a Query')
...
if context_df["Score"].iloc[0] <= 0.6:
    ...
    print(response_rows,type(response_rows))
    ...
    print(matching_rows_with_links)
else:

    #st.write("No Exact matching report found and below are the few suggestion tables")
    print("No Exact matching report found and below are the few suggestion tables")
