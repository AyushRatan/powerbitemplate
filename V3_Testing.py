...
if submit_button:
    ...
    if context_df["Score"].iloc[0] <= 0.45:
        ...
        matching_rows_with_links['Links'] = matching_rows_with_links['Links'].apply(make_clickable)
        st.table(matching_rows_with_links.style.set_caption('Website Links'))
        # for i, row in matching_rows_with_links.iterrows():
        #     link = row['Links']
        #     st.markdown(f"<a href='{link}'>{link}</a>", unsafe_allow_html=True)
    else:
        response_rows_2 = openai_response_2(query, tables_df)
        #print(response_rows_2)
        #st.write(tables_df)
        #print(response_rows_2)
        try:
            result_list_2 = ast.literal_eval(response_rows_2)
            # print("list", response_rows_2)
            # print("type", type(response_rows_2))
            ...
            st.write("No matching tables and columns found")
        #print(matching_rows)